...
Deno.test({
  name: "soliditySha3",
  fn() {
    tests.forEach(({ expected, values }) => {
      values.forEach((value) => {
        if (value.error || _.isArray(value)) {
          testing.assertThrows(soliditySha3.bind(null, value));
        } else {
          testing.assertEquals(soliditySha3(value), expected);
        }
      });
    });
  },
...
Deno.test({
  name: "soliditySha3 should hash mixed boolean values in any order",
  fn() {
...
Deno.test({
  name: "soliditySha3 should hash mixed string and number values in any order",
  fn() {
    testing.assertEquals(
      soliditySha3(
...
Deno.test({
  name: "soliditySha3 should hash mixed number types in any order",
  fn() {
    testing.assertEquals(
      soliditySha3(
...
Deno.test({
  name:
    "soliditySha3 should hash mixed number types addresses and boolean in any order",
  fn() {
    testing.assertEquals(
      soliditySha3(
        tests[5].values[0], // v: '56', t: 'uint8'
...
Deno.test({
  name:
    "soliditySha3 should hash mixed number arrays addresses and boolean in any order",
  fn() {
    testing.assertEquals(
      soliditySha3(
